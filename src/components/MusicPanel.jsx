...
const MusicPanel = ({ isCollapsed, onToggle }) => {
  const { musicTracks, setMusicTracks, saveHistory, layers, dimensions, setIsMusicEditorOpen, setEditingMusicTrackId, generateWav, addModArchiveArtist } = usePxShop();
  ...
  const handleRenameTrack = (trackId, newName) => {
    ...
    if (track && track.type !== 'group') {
      const originalExt = track.name.includes('.') ? '.' + track.name.split('.').pop() : (track.isSfx ? '.wav' : '.mod');
    ...
    <div style={{ display: 'flex', flexDirection: 'column', flex: isCollapsed ? 'none' : 1, borderBottom: '2px solid #222', minHeight: 0, background: isCollapsed ? 'transparent' : '#3d3d3d' }}>
      ...
      >
        <span style={{ fontWeight: 'bold', fontSize: '11px', textTransform: 'uppercase', color: isCollapsed ? '#aaa' : '#4CAF50', letterSpacing: '0.5px', display: 'flex', alignItems: 'center', gap: '8px' }}>
          <BsMusicNoteBeamed /> Music & SFX
        </span>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }} onClick={e => { if (isCollapsed) { onToggle(); } e.stopPropagation(); }}>
          {!isCollapsed && (
            <button onClick={addMusicGroup} title="Add Group" style={{ backgroundColor: '#ff9800', border: 'none', color: '#fff', padding: '4px 8px', borderRadius: '4px', cursor: 'pointer', display: 'flex', alignItems: 'center' }}>
              <BsFolder2Open />
            </button>
          )}
            ...
            musicTracks.map((track, index) => {
              ...
              return (
                <div key={track.id} style={{ marginLeft: track.groupId ? '15px' : '0', display: 'flex', flexDirection: 'column', gap: '4px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#111', padding: '6px 8px', borderRadius: '4px', border: track.isSfx ? '1px solid #e040fb' : '1px solid #4CAF50', gap: '8px' }}>
                    {renamingTrackId === track.id ? (
                      <div style={{ display: 'flex', alignItems: 'center', gap: '4px', flex: 1 }}>
                        <input
                          ...
                          style={{
                            ...
                            background: '#222',
                            color: '#fff',
                            border: track.isSfx ? '1px solid #e040fb' : '1px solid #4CAF50',
                            borderRadius: '3px',
                            padding: '2px 6px',
                            fontSize: '12px',
                            outline: 'none'
                        ...
                        />
                        <button onClick={() => handleRenameTrack(track.id, renamingName)} style={{ background: track.isSfx ? '#e040fb' : '#4CAF50', border: 'none', color: '#fff', borderRadius: '3px', padding: '4px', cursor: 'pointer', display: 'flex', alignItems: 'center' }}>
                          <BsCheck size={14} />
                        </button>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'flex-start', alignItems: 'center', padding: '2px 0 6px 0' }}>
                    ...
                    >
                      ...
                      {musicTracks.filter(item => item.type === 'group').map(g => (
